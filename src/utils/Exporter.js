...
const generateHTML = (data) => {
    ...
    // Header Logic
    // Header Logic
    const headerClass = data.header?.style === 'solid' ? 'relative shadow-sm' : 'absolute top-0 left-0';
    const headerStyle = data.header?.style === 'solid' ? `background-color: ${data.header.bgValue || '#ffffff'}; color: ${data.header.textColor || '#333333'};` : 'color: white;';
    const headerMixBlend = data.header?.style === 'overlay' ? 'mix-blend-difference' : '';

    // Layout Logic
    const isCenter = data.header?.layout === 'center';
    const headerContainerClass = `flex items-center max-w-7xl mx-auto relative ${isCenter ? 'justify-end' : 'justify-between'}`;
    const logoTailwind = isCenter ? 'absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2' : 'relative';

    const logoHtml = data.header?.logoUrl
        ? `<div class="z-50 ${logoTailwind}"><img src="${data.header.logoUrl}" alt="${data.siteTitle}" style="height: ${data.header.logoHeight || 40}px; object-fit: contain;"></div>`
        : `<div class="z-50 ${logoTailwind}"><div class="font-bold text-lg tracking-widest uppercase opacity-90 ${headerMixBlend}">${data.siteTitle}</div></div>`;
